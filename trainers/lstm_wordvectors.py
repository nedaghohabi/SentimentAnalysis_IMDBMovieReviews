...
from model_factory.lstm import LSTMWithW2VEmbedding
...
def parse_args():
    ...
    parser.add_argument("--freeze_embeddings", 
                        action="store_true", help="Freeze word2vec embeddings")
    
    parser.add_argument("--bidirectional", 
                        action="store_true", help="Use bidirectional LSTM")
    
    parser.add_argument("--use_attention", 
                        action="store_true", help="Use attention mechanism")
...
def tokenize_vector(word2idx, doc):
...
def main():
    set_seed(42)
    args = parse_args()
    print(args.bidirectional, args.use_attention)
    ...
    X_train = [tokenize_vector(word2idx, text) for text in train_sentences]
    X_val = [tokenize_vector(word2idx, text) for text in val_sentences]
    X_test = [tokenize_vector(word2idx, text) for text in test_sentences]
    ...
    torch.save(runner.model.state_dict(), osp.join(args.output_dir, "lstm_model.pth"))
    with open(osp.join(args.output_dir, "word2idx.json"), "w") as f:
        json.dump(word2idx, f)
    logger.log_message(f"Model and embedding dict saved to {args.output_dir}")
