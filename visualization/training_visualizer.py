...
class TrainingPlotter:
    def __init__(self, metrics_names, save_path=None):
        ...
        if self.num_metrics == 0:
            ...
            self.fig, (*metric_axes, self.loss_ax) = plt.subplots(ncols=self.num_metrics + 1, nrows=1)
        self.metric_axes = {name: ax for name, ax in zip(metrics_names, metric_axes)}
    ...
    def update(self, epoch, train_metrics, val_metrics, train_loss, val_loss):
        for name in self.metrics_names:
            ...
            self._update_ax(self.metric_axes[name], train_line, val_line,
                            self.train_metrics[name], self.val_metrics[name], epoch)
